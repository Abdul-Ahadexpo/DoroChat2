...
import { getAllUserStatuses } from '../services/firebase';
...
const MessageList: React.FC<MessageListProps> = ({ messages, roomId, onReply }) => {
  ...
  const handleProfileClick = (senderId: string, senderName: string, senderProfileImage?: string) => {
    // Get the specific user's custom status from Firebase
    const userStatus = userStatuses[senderId] || { text: '', emoji: '', expiresAt: null };
